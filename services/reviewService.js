...
export const canUserReview = async (userId, productId) => {
    ...
    if (!hasPurchased) return { canReview: false, reason: "You must purchase this product before reviewing" };

    const existingReview = await Review.findOne({ user: userId, product: productId });
    if (existingReview) return { canReview: false, reason: "You have already reviewed this product" };
