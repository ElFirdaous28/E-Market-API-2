...
class DiscountService {
    ...
    async validateCouponForUser(couponCode, userId, totalAmount) {
        ...
        if (totalAmount < coupon.minimumPurchase) {
            return {valid:false,message: `Minimum purchase amount is ${coupon.minimumPurchase}`};
        }
    ...
    async applyCouponToOrder(couponCode, userId, orderId) {
        const coupon = await Coupon.findOne({code: couponCode.toUpperCase() });
        const existingUsage = coupon.usedBy.find(usage => usage.user.toString() === userId);
