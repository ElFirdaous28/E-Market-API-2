...
describe("Review API", function () {
    ...
    before(async () => {
        ...
        [normalUser] = await userFactory(1,{email:"user@test.com", password: "user123"});
    ...
    describe("Create a review without purchase", () => {
        it("Should reject review without purchase", async () => {
            ...
            expect(res.body.error).to.equal("You must purchase this product before reviewing");
        });
    });
    ...
    describe("List reviews for one product", () => {
        beforeEach(async () => {
            await Review.create([
                { user: normalUser._id, product: testProduct._id, rating: 4, comment: "Good product" },
                { user: user2._id, product: testProduct._id, rating: 5, comment: "Excellent" },
                { user: adminUser._id, product: testProduct._id, rating: 3, comment: "Average" }
            ]);
        });
    ...
    describe("Updated average rating", () => {
        beforeEach(async () => {
            await Review.create([
                { user: normalUser._id, product: testProduct._id, rating: 4, comment: "Good", status: "approved" },
                { user: user2._id, product: testProduct._id, rating: 5, comment: "Excellent", status: "approved" }
            ]);
        });

        it("should calculate correct average rating", async () => {
            const { averageRating, totalReviews } = await Review.calculateAverageRating(testProduct._id);
