...
notificationEmitter.on("newOrder", async ({ orderId, buyerId, sellerId }) => {
  try {
    ...
    console.error("Erreur lors de la création des notifications et emails de commande :", error);
  }
});

// Quand une commande est supprimée
notificationEmitter.on("orderDeleted", async ({ orderId, buyerId, sellerId }) => {
  try {
    // Notifications
    await Notification.create({ userId: sellerId, message: `Commande ${orderId} a été supprimée` });
    await Notification.create({ userId: buyerId, message: `Votre commande ${orderId} a été supprimée` });
    ...
    console.log("Notifications et emails créées pour la suppression de la commande !");
  } catch (error) {
    console.error("Erreur lors de la création des notifications et des emails de suppression de commande :", error);
  }
});
