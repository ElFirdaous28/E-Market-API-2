...
export const productFactory = async (count = 1, overrides = {}) => {
    ...
    if (!sellers.length) throw new Error("No sellers found — seed sellers first!");
   ...
   for (let i = 0; i < count; i++) {
    ...
    } else if (Array.isArray(overrides.categoryIds) && overrides.categoryIds.length) {
      categoryIds = overrides.categoryIds;
    } else if (categories.length) {
      const howMany = faker.number.int({ min: 0, max: Math.min(2, categories.length) });
      for (let j = 0; j < howMany; j++) {
        categoryIds.push(faker.helpers.arrayElement(categories)._id);
      }
    }

    const price = overrides.price ?? Number(faker.number.float({ min: 5, max: 500, precision: 0.01 }).toFixed(2));
    let ex_price = overrides.ex_price ?? undefined;
    if (ex_price == null && faker.datatype.boolean() && price > 5) {
      const increasePercent = faker.number.float({ min: 0.05, max: 0.4, precision: 0.01 }); // 5%..40% higher
    ...
    const doc = {
      ...
      categories: categoryIds.map((c) => (typeof c === 'string' ? mongoose.Types.ObjectId(c) : c)),
      primaryImage: overrides.primaryImage || `/uploads/products/${faker.string.uuid()}.webp`,
      secondaryImages:
        Array.isArray(overrides.secondaryImages) && overrides.secondaryImages.length
          ? overrides.secondaryImages
          : Array.from({ length: faker.number.int({ min: 0, max: 3 }) }, () => `/uploads/products/${faker.string.uuid()}.webp`),
      published: typeof overrides.published === 'boolean' ? overrides.published : faker.datatype.boolean(),
